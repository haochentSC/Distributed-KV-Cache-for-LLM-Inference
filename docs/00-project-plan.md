...
| E | **AWS infra: etcd 3-node + Spot nodes + IAM + S3 cold tier** | 🟡 authored, pending `apply` | 0027, 0028 | **Code done (3 commits, this session):** S3 cold tier (spill-on-evict + Fetch read-through, ADR 0027, tested locally); full Terraform (`terraform/bootstrap` + `terraform/cluster`) — single-AZ VPC, 3-node static-IP etcd, Spot cache nodes running the ECR image under systemd (SIGTERM→drain), least-priv IAM, S3 cold bucket + DynamoDB/S3 remote state, CloudWatch logs+alarms; `scripts/push-image.sh` + `scripts/aws-chaos.sh`. Layout/decisions in ADR 0028; runbook `terraform/README.md`. **Remaining:** HC does Stage-0 onboarding (account/CLI/Terraform), then `fmt`/`validate`/`apply`; verify etcd quorum + membership + cold-tier round-trip + AWS chaos (instance-kill + `tc`/`iptables`). Never auto-`apply`. |
