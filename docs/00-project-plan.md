...
> **Status (2026-06-09): single-node done (ADR 0031); distributed prep done, no spend (ADR 0032).**
> Local single-node TTFT is measured + decomposed — the cache loses at ≤3B for *environmental*
> reasons (WSL2 unpinned memory, Python/protobuf hot path, throttled KV cache), deficit closing with
> model size. HC chose the headline to be a **30B-class model with tensor parallelism (TP=4)** on a
> `g5.12xlarge` (4× A10G). Everything for that paid window is now built and locally verified: the
> connector keys each TP rank's KV-head shard distinctly (`shard_model_id`, scheduler checks presence
> under canonical rank 0, ADR 0032); a cost-guarded GPU node in Terraform (`gpu_count` defaults to 0,
> so `apply` never bills it); a TP-aware distributed driver (`connector/scripts/run_distributed_benchmark.py`);
> and `loadgen -verify-coldtier`. The run itself is a runbook: `docs/benchmarks/aws-batch-runbook.md`
> (bundled with AWS chaos, CloudWatch, and the 5a/5b cluster re-run; ~$6–20, GPU torn down after).
